...
with its \newtermdef{accessors} (methods to access or modify and attribute) and default values.
\begin{center}
	\begin{tabular}{llc}
...
\noindent
The expression {\tt PSE[\class{Element}::O]} returns an instance of class
...
\noindent
Now we have to assign the correct coordinates to the two hydrogen atoms.
the method \method{setPosition} takes an instance of \class{Vector3} as an
argument. This class is used to represent coordinates and vectors in
three--dimensional space. We can assign the coordinates as follows:
...
\noindent
The \method{setPosition} method takes an objects of type \class{Vector3} as an
argument. This class is used throughout BALL to represent coordinates and
vectors. As shown in this example, an object of type \class{Vector3} can be
constructed from three floating point numbers which represent the x, y, and z
coordinates.
...
\class{Vector3}.
The vector class provides a powerful functionality. Due to operator overloading
the syntax is very readable. The length of a vector is returned by the
