...
The use of the stream operators is common to all file formats in BALL (see
also the first example). So you can easily replace the different file formats
...
This leads to a difference in the interface. A typical for loop using STL
...
string\_list}). The method {\tt begin()} returns an iterator that points to the
first element of the list. In the {\tt for} loop we increment the iterator
until it equals the iterator returned by {\tt end()}. This method returns a
past--the--end iterator, \ie the value an iterator assumes that is incremented
while pointing to the last element of the container. In the body of the {\tt
for} loop, we access the list element the iterator points to using the {\tt *}
operator.

Traversing a BALL kernel data structure is as simple as traversing a list with
STL. But unlike STL containers, the kernel objects may contain a variety of
different objects. SO we have to define over which objets we want to iterate.
