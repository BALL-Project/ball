...
namespace BALL  
  ...
  const char *MolecularDynamics::Option::MAXIMAL_NUMBER_OF_ITERATIONS =
                                                "maximal_number_of_iterations";

  const char *MolecularDynamics::Option::NUMBER_OF_ITERATION =
                                                "number_of_iteration";

  const char *MolecularDynamics::Option::ENERGY_OUTPUT_FREQUENCY =
                                                "energy_output_frequency";

  const char *MolecularDynamics::Option::SNAPSHOT_FREQUENCY =
                                                "snapshot_frequency";

  const char *MolecularDynamics::Option::TIME_STEP =
                                                "time_step";

  const char *MolecularDynamics::Option::REFERENCE_TEMPERATURE =
                                                "reference_temperature";

  const char *MolecularDynamics::Option::BATH_RELAXATION_TIME =
                                                "bath_relaxation_time";
  const char *MolecularDynamics::Option::CURRENT_TIME =
                                                "current_time";
  ...
  const float MolecularDynamics::Default::TIME_STEP = 0.0005;

  const float MolecularDynamics::Default::REFERENCE_TEMPERATURE = 300.0; 
  
  const float MolecularDynamics::Default::BATH_RELAXATION_TIME = 0.2;   

  const float MolecularDynamics::Default::CURRENT_TIME = 0.0;   




  const char* MolecularDynamics::error_message_[] =
		{
     "Unknown error.",
     "Not implemented."
    };
  ...
  MolecularDynamics::MolecularDynamics(ForceField &myforcefield)
   ...
   if(!valid_)
     ...
     Log.level(LogStream::ERROR) 
                           <<   "Setup of instance of class 'MolecularDynamics' has failed."
                           << endl; 
     }
   }
  ...
  bool MolecularDynamics::setup(ForceField &myforcefield, SnapShotManager *ssm,
    ...
    if(myforcefield.isValid() == false)
      {
      // The setup has failed for some reason. Output an error message.
      Log.level(LogStream::ERROR) << "Setup of instance of class 'MolecularDynamics' has failed."
                                 << endl;
    ...
    maximal_number_of_iterations_ = options.getInteger(
                                 MolecularDynamics::Option::MAXIMAL_NUMBER_OF_ITERATIONS); 
    ...
    options.setDefaultReal(MolecularDynamics::Option::TIME_STEP,
                          MolecularDynamics::Default::TIME_STEP);
    ...
    reference_temperature_ = 
                           options.getReal(MolecularDynamics::Option::REFERENCE_TEMPERATURE);



    // The current time of the simulation. This is useful when several MD runs shall be done.
    options.setDefaultReal(MolecularDynamics::Option::CURRENT_TIME,
                           MolecularDynamics::Default::CURRENT_TIME); 
    ...
    snapshot_frequency_ = 
                          options.getInteger(MolecularDynamics::Option::SNAPSHOT_FREQUENCY); 
    ...
    void MolecularDynamics::setReferenceTemperature(float temperature)
      {
      reference_temperature_ = temperature; 
      }

    // This methods resets the current time 
    void MolecularDynamics::setCurrentTime(float time)
      {
      current_time_ = time;
      }
    ...
    float MolecularDynamics::getTemperature() const 
      {
    ...
    // This method will be overwritten by a subclass
    void MolecularDynamics::simulateTime(float time,bool restart)
      {
      }
    ...
    void MolecularDynamics::updateInstantaneousTemperature()
      ...
      if(force_field_ptr_->periodic_boundary.isEnabled() == true)
        ...
        for(atom_it =  atom_vector_.begin(); atom_it != atom_vector_.end();
                                                                        ++atom_it)
          {
          no_of_atoms++; 
