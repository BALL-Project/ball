			...
			parameters_initialized_(false)
  {
		...
		throw(Exception::TooManyErrors)
	{
		...
		if (parameters_.getFilename() != filename || !parameters_initialized_)
		{
			...
			// retrieve the equivalent atom types
			equivalent_types_.setFilename(filename);
			equivalent_types_.init();

			// this is the first time, parameters_ was initialized
			...
			// TODO ?????: Iterator ueber global_options.options
	...
	void AmberFF::exportParmFile(String outfilename) const	
		throw(Exception::FileNotFound)
	{
		// open file
		File outfile(outfilename, std::ios::out);	
		if (!outfile.good()) 
		{
			throw Exception::FileNotFound(__FILE__, __LINE__, outfilename);
		}	
		
		// write the title line
		outfile << "PARM96 for DNA, RNA and proteins with TIP3P Water generated by BALL. USE SCEE=1.2 in energy progs" << endl;	
	
		// write the atomtypes and hydrophilic atoms
		parameters_.exportParmFile(outfile);

		// write the bond length parameters	
		AmberStretch* amber_stretch_component = (AmberStretch*) getComponent("Amber Stretch");
		amber_stretch_component->exportParmFile(outfile);

		// write the bond angle parameters
		AmberBend* amber_bend_component = (AmberBend*) getComponent("Amber Bend");
		amber_bend_component->exportParmFile(outfile);
	
		// write the dihedral and improper dihedral parameters
		AmberTorsion* amber_torsion_component = (AmberTorsion*) getComponent("Amber Torsion");
		amber_torsion_component->exportParmFile(outfile);
		
		// write H-bond 10-12 potential parameters
		AmberNonBonded*  amber_non_bonded_component = (AmberNonBonded*) getComponent("Amber NonBonded");
		amber_non_bonded_component->exportHBondParmFile(outfile);

		// write the equivalent atom types
		equivalent_types_.exportParmFile(outfile);
	
		amber_non_bonded_component->exportVdWParmFile(outfile);
		outfile << "END " << endl;
	}
