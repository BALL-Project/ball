...
void AtomProperties::applyClicked()
{
	try
	{
		atom_->setName(String(name_edit->text().ascii()));
		atom_->setType(String(type_edit->text().ascii()).toShort());
		atom_->setTypeName(String(type_name_edit->text().ascii()));
		atom_->setCharge(String(charge_edit->text().ascii()).toFloat());
		atom_->setRadius(String(radius_edit->text().ascii()).toFloat());
		atom_->setElement(PTE[(String(element_box->currentText().ascii()))]);
	}
