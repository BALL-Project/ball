...
// vi: set ts=2:
//
// $Id: standardColorProcessor.C,v 1.2 2003/08/26 09:18:01 oliver Exp $
...
namespace BALL
{
	namespace MOLVIEW
	{
		...
		const String& ResidueNameColorProcessor::calculateKey(const Atom& atom) const
			throw()
		{
			...
			throw()
		{
				...
				Log.info() << "Destructing object " << (void *)this 
									 << " of class " << RTTI::getName<AtomChargeColorProcessor>() << std::endl;
			#endif 
		}

		void AtomChargeColorProcessor::clear()
			throw()
		{
		...
		void AtomChargeColorProcessor::set(const AtomChargeColorProcessor& color_processor)
			throw()
		{
			positive_color_.set(color_processor.positive_color_);
			neutral_color_.set(color_processor.neutral_color_);
			negative_color_.set(color_processor.negative_color_);
		}

		const AtomChargeColorProcessor& AtomChargeColorProcessor::operator =
			(const AtomChargeColorProcessor& color_processor)
			throw()
		{
			set(color_processor);
			return *this;
		...
		void AtomChargeColorProcessor::swap(AtomChargeColorProcessor& color_processor)
			throw()
		{
			positive_color_.swap(color_processor.positive_color_);
			neutral_color_.swap(color_processor.neutral_color_);
			negative_color_.swap(color_processor.negative_color_);
		}

		void AtomChargeColorProcessor::visit(Atom& atom)
		{
			...
			throw()
		{
			#ifdef BALL_VIEW_DEBUG
				Log.info() << "Destructing object " << (void *)this 
									 << " of class " << RTTI::getName<AtomDistanceColorProcessor>() << std::endl;
			#endif 
		}

		void AtomDistanceColorProcessor::clear()
			throw()
		{
		...
		void AtomDistanceColorProcessor::set(const AtomDistanceColorProcessor& color_processor)
			throw()
		{
			clear();
		...
		const AtomDistanceColorProcessor& AtomDistanceColorProcessor::operator =
			(const AtomDistanceColorProcessor& color_processor)
			throw()
		{
			set(color_processor);
			return *this;
		...
		void AtomDistanceColorProcessor::swap(AtomDistanceColorProcessor& color_processor)
			throw()
		{
			float temp = distance_;
			distance_ = color_processor.distance_;
			color_processor.distance_ = temp;

			null_distance_color_.swap(color_processor.null_distance_color_);
			full_distance_color_.swap(color_processor.full_distance_color_);
		}
		...
		Processor::Result AtomDistanceColorProcessor::operator() (Composite& composite)
		{
		...
		Processor::Result AtomDistanceColorProcessor::operator() (Atom& atom)
		{
