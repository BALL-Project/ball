...
//	double largest_eigenvalue=eigenValues(first_eigenvector);
	...
	// use the significant eigenvectors as loadings that confer an importance for each feature for a latent variable
	loadings_.ReSize(descriptor_matrix_.Ncols(),cols); 
	
	// the latent variables, which are generated by use of the loading vectors (==the selected eigenvectors)
	latent_variables_.ReSize(descriptor_matrix_.Nrows(),cols);
	
	int c=1;
 	for (int i=first_eigenvector; i>=last_eigenvector ; i--)
 	{
		latent_variables_.Column(c)=descriptor_matrix_*eigenVectors.Column(i);
		loadings_.Column(c)=eigenVectors.Column(i);
		c++;
 	}
