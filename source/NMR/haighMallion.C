...
//// diverse Funktionen
	...
	float
	getLambda (const Vector3 & v1, const Vector3 & v2)
	{
		if (v2.x == 0)
			if (v2.y == 0)
				if (v2.z == 0)
...
//FinishFunktion
	...
	HaighMallionShift::finish ()
	{
		...
		int
			index, found;
		float
			hshift, a, a_eins, a_zwei, h, f, ts, gs, ring_flaeche;
		Vector3
			n0, eins_zwei, eins, zwei, v, v_ring, links, mitte, rechts, ein_vector;
		Vector3 *
			vector_feld_;
				...
				if ((*proton_iter)->getResidue () != residue_)
				{
					...
					do
					{
						...
						for (counter2 = 0; counter2 < vcounter; counter2++)
						{
							...
							if (getLambda (((v_ring - eins) % (v_ring - zwei)), normal_) > 0)
								f *= -1;
