	...
	BALL
{
	...
	const float
		radius1 = 1.39e-10;					//Schleifenradius gross
	const float
		radius2 = 1.182e-10;				//Schleifenradius klein
	const float
		n1 = 3;											// number of aromatic electrons /2  for six-memeberd rings
	const float
		n2 = 3;											//    -- " --                       for five-membered rings

	// diverse Funktionen

	Vector3
	kreuz (const Vector3 & v1, const Vector3 & v2)
	{
		Vector3 *
			erg = new Vector3;
	...
	float
	betrag (const Vector3 & v)		// berechnet Betrag eines Vektors
	{
		float
			e = sqrt ((v * v));
		return e;
	...
	float
	JohnsonBoveyShift::rf (float x, float y, float z)
	{
		...
		float
			alamb, ave, delx, dely, delz, e2, e3, sqrtx, sqrty, sqrtz, xt, yt, zt;
	...
	float
	JohnsonBoveyShift::rd (float x, float y, float z)
	{
		//Lokale Konstanten Definitionen :
	...
	float
	JohnsonBoveyShift::ellf (float phi, float ak)
	{
		/*
	...
	float
	JohnsonBoveyShift::elle (float phi, float ak)
	{
		/*
	...
	JohnsonBoveyShift::finish ()
	{
		...
		int
			index, found;
		float
			wert, p, z, lambda, k, e, hshift;
		Vector3
			links, mitte, rechts, v;
		Vector3 *
			vector_field_;
		...
		for (proton_iter = proton_list_.begin (); proton_iter != proton_list_.end (); ++proton_iter)	//Protonen Iterator
		{
				...
				if ((*proton_iter)->getResidue () != residue_)
				{
					...
					do
					{
						...
						hshift = ((VACUUM_PERMEABILITY * n_ *
											 ELEMENTARY_CHARGE * ELEMENTARY_CHARGE) / (4 * PI * 6 * PI * ELECTRON_MASS * radius_));
			...
			//cout << endl <<"JB:Residue :"<<(*proton_iter)->getResidue()->getName()<<" Atom :"<<(*proton_iter)->getName()<< " chemical_shift :" << shift_;
