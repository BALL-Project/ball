// $Id: NMRSpectrum.C,v 1.1 2000/07/03 20:02:17 oliver Exp $
...
void NMRSpectrum::plot_spectrum(String file_name)
	...
	for (x=min;x<=max;x+=y)  
		...
		for(peak_iter2=spectrum_->begin();peak_iter2!=spectrum_->end();++peak_iter2)
			...
			ts=height/(1+((ppm*2*Constants::PI*1e6-omega*2*Constants::PI*1e6)*(ppm*2*Constants::PI*1e6-omega*2*Constants::PI*1e6))*4/breadth);
			gs+=ts;
...
void make_difference(float diff,String a,String b,String out)
	...
	do
		...
		for (iter=liste_b.begin();iter!=liste_b.end();++iter)
			{
			if ( (atom_name==(*iter).name)&&( (shift-(*iter).shift <diff)&&(shift-(*iter).shift>-diff) )  ) found =1;
			}
		if (!found)
			{
			outfile << atom_name <<" "<< shift <<endl;
