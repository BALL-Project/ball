...
namespace BALL
{
	...
	void RSConstructor::buildRegularPolygon(vector<Atom*>& ring, Size& z)
	{
		...
		cout << "\t-*-[RSConstructor]:\t (buildRegularPolygon):\t done." << endl << endl;
	}
	...
	{
		cout << "\t-*-[RSConstructor]:\t (attachFused):\t\t started." << endl;
		...
		vector<Atom*>::size_type q_first;
		vector<Atom*>::size_type q_second;
		...
		for(vector<RingInfo>::size_type k = 0; k != ringsystem.size(); k++)
		{
			...
			if((ringsystem[k].ring != ring) && ringsystem[k].ring[1] -> hasProperty(deposited) && ringsystem[k].ring[2] -> hasProperty(deposited) && ringsystem[k].ring[3] -> hasProperty(deposited))
			{
		...
		if(first_found && second_found)
		{
			...
			if(!(ring[border] -> hasProperty(deposited)))
			{
				for(vector<Atom*>::size_type i = border; i != q_first; i++)
				{
					...
					if(!(ring[i] -> hasProperty(deposited)))
					{
						ring[i] -> setPosition(pos_vec3);
					}
					ring[i] -> setProperty(deposited);
			...
			cout << "\t-*-[RSConstructor]:\t (attachFused):\t done." << endl << endl;
		}
		...
		{
			cout << "\t-*-[RSConstructor]:\t (attachFused):\t aborted. Ring appended." << endl << endl;
	...
	void RSConstructor::attachBridged(vector<Atom*>& ring, vector<RingInfo>& ringsystem)
	{
		...
		cout << "\t-*-[RSConstructor]:\t (attachBridged):\t started." << endl << endl;
		...
		{
			//	the other ring must already be constructed (deposited) and may not be the ring that is to be constructed itself
			if((ringsystem[j].ring != ring) && (ringsystem[j].ring[0] -> hasProperty(deposited)))
			{
		...
		for(Size i = 0; i != q_s.size(); i++)
		{
			for(Size j = 0; j != q_s.size(); j++)
			{
				...
				{
					if(ring[q_s[i]+1] -> hasProperty(deposited))
					{
		...
		if(ring[q_first + 1] -> hasProperty(deposited))
		{
			TVector3<float> temp_vec = (ring[q_first] -> getPosition() - ring[q_second] -> getPosition());
		}
			...
			for(vector<Atom*>::size_type i = start ; i != stop  ; i++)
			{
				...
				if(i != q_second && i != q_first)
				{
					for(Size j = 0; j != ringsystem.size(); j++)
					{
						...
						{
							if(!(ringsystem[j].ring[k] -> hasProperty(deposited)))
							{
								undepos_ring = true;
							}
						}
		...
		cout << "\t-*-[RSConstructor]:\t (attachBridged):\t done." << endl;
	}
	...
	{
		cout << "\t-*-[RSConstructor]:\t (attachSpiro):\t\t started." << endl << endl;

		vector<Atom*> ring = ring_info.ring;

		// finding the shared atom

		vector<Atom*>::size_type keep_i;
		vector<Atom*>::size_type init_poSize1;
		vector<Atom*>::size_type init_poSize2;
		vector<vector<Atom*> >::size_type keep_j;
	...
	//	interface for a Ring Template Database  // ???? was ist hiermit?
	void RSConstructor::attachTemplate(vector<Atom*>& /* ring */)
	{
		cout << "\t-*-[RSConstructor]:\t (attachTemplate):\t started." << endl << endl;
	}
	...
	void RSConstructor::constructRS(vector<RingInfo>& analysed_rings, Size& z)
	{
		...
		{
			//     cout << " assignment = " << analysed_rings[i].assignment << endl;
			if (analysed_rings[i].assignment == 5)				//assigment = CORE
			{
		...
		for(vector<RingInfo>::size_type i = 0; i != analysed_rings.size(); i++)
		{
			if(analysed_rings[i].assignment == 1)	//assignment = TEMPLATE
			{
		...
		for(vector<RingInfo>::size_type i = 0; i != analysed_rings.size(); i++)
		{
			if(analysed_rings[i].assignment == 4)		//assignment = SPIRO
			{
				attachSpiro(analysed_rings[i], analysed_rings);
			}

			if(analysed_rings[i].assignment == 2)		//assignment = FUSED
			{
		...
		for(vector<RingInfo>::size_type i = 0; i != analysed_rings.size(); i++)
		{
			...
			if(analysed_rings[i].assignment == 3)		//assignment = BRIDGED
			{
