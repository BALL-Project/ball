...
namespace BALL
{
	...
	vector<vector<Size> > ChainBuilder::calcAdjMatrix(vector<Atom*>& core_chain_atoms)
	{
		...
		cout << "\t-*-[ChainHandler]:\tcalcAdjMatrix started." << endl << endl;
		...
		cout << "\t-*-[ChainHandler]:\tcalcAdjMatrix done." << endl << endl;
	...
	void ChainBuilder::findEdges(vector<Atom*>& chain_area)
	{
		...
		for(vector<Atom*>::size_type i = 0; i != chain_area.size(); i++)
		{
			...
			{
				if(bond_it -> getPartner(*chain_area[i]) -> hasProperty(core_chain))
				{
	...
	vector<Atom*> ChainBuilder::findPath(Atom*& first_edge, Atom*& second_edge,vector<Atom*>& chain_area, vector<vector<Size> >& adj_matrix)
	{
		...
		Size t;	//	starting poSize for the traceback after the end atom has been found
		...
		{
			val.push_back(99999);		//	"99999" means the atom has not been visited yet
		...
		{
			//cout << "["<< traceback -> getName()<< "]-" << flush;
			path.push_back(traceback);

			traceback = prev_nodes[getIndex(traceback, chain_area)];
		}
		//cout << "-["<< chain_area[start] -> getName()<< "]" << endl;
		path.push_back(chain_area[start]);

		return path;
	}

	bool compare(const vector<Atom*>& x, const vector<Atom*>& y)
	{
	...
	{
		cout << "\t-*-[buildChains]:\t findChains called." << endl;
		...
		for(vector<Atom*>::size_type i = 0; i != chain_area.size(); i++)
		{
			for(vector<Atom*>::size_type j = 0; j != chain_area.size(); j++)
			{
				if(i != j)
				{
					...
					if(chain_area[i] -> hasProperty(edge)  && chain_area[j] -> hasProperty(edge))
					{
		...
		sort(chains.begin(), chains.end(), compare);
		...
		for(Size i = 0; i != chains.size(); i++)
		{
			...
			if(chains[i].size() == chains[0].size())
			{
				...
				{
					if(bond_it -> getPartner(*chains[i][0]) -> hasProperty(in_ring))
					{
						adj_rings++;
					}
				}
				...
				{
					if(bond_it -> getPartner(*chains[i][chains[i].size() -1]) -> hasProperty(in_ring))
					{
						adj_rings++;
					}
				}
		...
		{
			chains[keep_i][i] -> setProperty(built_in_chain);
		}
		...
		for(Size i = 1; i != chains.size();)
		{
			...
			while(!(increment_i))
			{
				if(!(chains[i][j] -> hasProperty(built_in_chain)))
				{
					chains[i][j] -> setProperty(built_in_chain);
	...
	vector<vector<Atom*> > ChainBuilder::buildChains(System& molecule_sys)
	{
		...
		{
			if(atom_it -> hasProperty(core_chain))
			{
		...
		if(core_chain_atoms.size() > 0)
		{
			...
			cout << "\t-*-[buildChains]:\t" << chain_areas.size() << " chain areas found." << endl << endl;
			...
			sort(all_chains.begin(), all_chains.end(), compare);
		}
