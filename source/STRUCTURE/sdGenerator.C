...
namespace BALL
{
	...
	vector<vector<Atom*> > SDGenerator::sequenceRings(vector<vector<Atom*> >& ringsystem)
	{
		...
		for(vector<vector<Atom*> >::size_type j = 0; j != ringsystem.size(); j++)
		{
			...
			Atom* next_a;	//	the next atom to be considered
			Atom* curr_a;	//	the atom that is currently beeing considered
	...
	void SDGenerator::generateSD(System& molecule_sys)
	{
		...
		//   cout << endl << chains.size() << " chains found : " << endl;
		//   for(Size i = 0; i != chains.size(); i++)
		//   {
		//     for(Size j = 0; j != chains[i].size(); j++)
		//     {
		//       cout << "[" << chains[i][j] -> getName() << "] " << flush;
		//     }
		//     cout << endl;
		//   }
		//   cout << endl;
