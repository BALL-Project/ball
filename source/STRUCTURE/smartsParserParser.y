...
expression:
		node { $$ = $1; }
	| node bond expression { new SmartsParser::SPBond($1, $3, $2); $$ = $1; }
	| node expression { new SmartsParser::SPBond($1, $2); $$ = $1; }
	;

node:
		atom { $$ = $1; }
	| node '(' expression ')' { new SmartsParser::SPBond($3, $1, 1); $$ = $1; }
	| node '(' bond expression ')' { new SmartsParser::SPBond($4, $1, $3); $$ = $1; }
	;
...
atom:
		org_subset_atom_symbol opt_connection_list 
		{
			...
			if ($2 != 0) 
			{ 
				SmartsParser::state.current_parser->createBonds($1, $2); 
			} 
		}
	|	'[' opt_isotope atom_symbol opt_chirality opt_hydrogens opt_charge ']' opt_connection_list
		{ 
			...
			if ($5 != 0)
			{
				for (int i = 0; i < $5; i++)
				{
					new SmartsParser::SPBond(SmartsParser::state.current_parser->createAtom("H"), $3);
				}
			}
			if ($4 != 0)
			{	
			...
			if ($8 != 0) 
			{ 
				SmartsParser::state.current_parser->createBonds($3, $8); 
				delete  $8;
			} 
			$$ = $3; 
		}
	| '[' opt_isotope TK_HYDROGEN opt_charge ']' 
		{ 
			$$ = SmartsParser::state.current_parser->createAtom("H"); 
			$$->setFormalCharge($4); 
			$$->setIsotope($2);
			$$->setInBrackets(true); 
		}
...
org_subset_atom_symbol:
		TK_ORG_SUBSET_ATOM { $$ = SmartsParser::state.current_parser->createAtom(yylval.text); }
	;

atom_symbol:
		org_subset_atom_symbol { $$ = $1; }
	|	TK_ATOM { $$ = SmartsParser::state.current_parser->createAtom(yylval.text); }
	;
