...
int main(int argc, char** argv)
{
	...
	parpars.registerMandatoryParameter("rec", "input receptor file", INFILE);
	parpars.registerMandatoryParameter("lig", "input ligand file", INFILE);
	parpars.registerOptionalParameter("cr", "charge rules", INFILE);
	parpars.registerOptionalParameter("pr", "radius rules for the polar solvation", INFILE);
	parpars.registerOptionalParameter("lj", "use FILE for LJ parameters", INFILE);
	parpars.registerOptionalParameter("op", "read options from FILE (command line overrides!)", INFILE);
	parpars.registerOptionalParameter("v", "verbosity to level", BALL::INT);
	parpars.registerOptionalParameter("s", "scaling factor for receptor charges", BALL::DOUBLE);
	parpars.registerOptionalParameter("t", "scaling factor for ligand charges", BALL::DOUBLE);
