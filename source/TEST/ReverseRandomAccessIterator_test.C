...
// vi: set ts=2:
//
// $Id: ReverseRandomAccessIterator_test.C,v 1.1 2003/05/28 16:51:10 oliver Exp $
//
...
typedef ReverseRandomAccessIterator<vector<float>, float, VectorIteratorPosition_, VectorIteratorTraits_<float> > MyIterator;

START_TEST(ReverseRandomAccessIterator, "$Id: ReverseRandomAccessIterator_test.C,v 1.1 2003/05/28 16:51:10 oliver Exp $")
...
CHECK(ReverseRandomAccessIterator())
...
CHECK(ReverseRandomAccessIterator(const Container& container))
...
CHECK(operator - (ReverseRandomAccessIterator))
