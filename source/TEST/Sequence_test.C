...
CHECK(Sequence(const String& name, const String& sequence))
	String aa_seq = "ALIMENTE";
	seqp= new Sequence("test_seq", aa_seq);

	TEST_EQUAL( seqp->getName(), "test_seq");
	TEST_NOT_EQUAL( seqp->getOrigin(), NULL);
	TEST_EQUAL(Peptides::GetSequence(*(dynamic_cast<Protein*>(seqp->getOrigin()))), "ALIMENTE");
RESULT
