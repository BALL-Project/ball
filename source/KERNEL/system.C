...
namespace BALL
{
	...
	namespace
	{
		...
		{
			// Idempotent re-entry (R36 BLOCKER): if c already has a row in
			// dst, its internal rows/edges were built on first adoption and a
			// later same-System reparent does not change them -- return the
			// existing row WITHOUT re-running the payload/children build, else
