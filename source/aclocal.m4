...
dnl vi: set ts=2:
dnl
dnl		$Id: aclocal.m4,v 1.50.2.6 2004/05/25 06:06:59 oliver Exp $
...
AC_DEFUN(CF_CHECK_FFTW_SUPPORT, [
	if test "${FFTW_SUPPORT}" = true ; then
		...
		if test "${FFTW_INCL}" != "" ; then
			CF_FIND_HEADER(FFTW_INCL_PATH, fftw.h, ${FFTW_INCL})
		else
			CF_FIND_HEADER(FFTW_INCL_PATH, fftw.h,)
		fi

		if test "${FFTW_INCL_PATH}" = "" ; then
			AC_MSG_RESULT((not found!))
			AC_MSG_RESULT()
			AC_MSG_RESULT(No FFTW header files found! Please specify the path to the FFTW header fftw.h)
		...
		CF_FIND_LIB(FFTW_LIB,libfftw, ${FFTW_LIBPATH})
		...
		AC_TRY_RUN(
			[
				...
				int main(int, char**)
				{
					std::ofstream os("fftwres.tmp", std::ios::out);
			...
			FFTW_PRECISION_TEST=true,
			DUMMY=0,
			DUMMY=0
		)
		EXIT_CODE=`${CAT} fftwres.tmp`
		${RM} fftwres.tmp 2>/dev/null
		if test "${EXIT_CODE}" = 1 -a "${FFTW_PRECISION_TEST}" = "true" ; then
			FFTW_PRECISION=float
		elif test "${EXIT_CODE}" = 2  -a "${FFTW_PRECISION_TEST}" = "true" ; then
			FFTW_PRECISION=double
		else
			AC_MSG_RESULT()
			AC_MSG_RESULT(FFTW_PRECISION_TEST=${FFTW_PRECISION_TEST})
