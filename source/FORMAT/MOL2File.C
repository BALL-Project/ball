// $Id: MOL2File.C,v 1.4 2000/05/23 05:44:33 oliver Exp $
...
namespace BALL 
{
	...
	void MOL2File::write(const System& system)
	{
		...
		if (number_of_bonds > 0)
		{
			...
			BALL_FOREACH_BOND(system, atom_it, bond_it)
			{
				...
				{
					// emit a warning
					Log.warn() << "MOL2File::write: could not write bond from " << bond_it->getFirstAtom()->getFullName() 
										 << " to " << bond_it->getSecondAtom()->getFullName() << " - not in system!" << endl;	
				}
			}
		}
    ...
    while (nextLine_())
    {
			...
			while (line_.hasPrefix(TRIPOS))
			{
				...
				{	
					...
					Log.warn() << "MOL2File::read: section ignored in line " << number_of_lines_ << ": " << RTI << endl;
					nextLine_();
				}
	...
	{
		Log.info() << "entering readAtomSection: line " << number_of_lines_ << endl;
	...
	{
		Log.info() << "entering readBondSection: line " << number_of_lines_ << endl;
		Size number_of_fields = 1;
		while (nextLine_() && (number_of_fields > 0) && !line_.hasPrefix(TRIPOS))
		{
			line_.trim();
			...
			if (number_of_fields > 0)
			{
				...
				else 
				{
					...
					bond.atom1 = fields[2].toUnsignedInt();
					bond.type = fields[3];
	...
	{
		Log.info() << "entering readSetSection: line " << number_of_lines_ << endl;
		Size number_of_fields = 1;
		while (nextLine_() && (number_of_fields > 0) && !line_.hasPrefix(TRIPOS))
		{
			line_.trim();
	...
	{
		Log.info() << "entering readMoleculeSection: line " << number_of_lines_ << endl;
		Size number_of_fields = 1;
		Size line_number = 1;
		while (nextLine_() && (number_of_fields > 0) && !line_.hasPrefix(TRIPOS) && (line_number <= 5))
		{
			...
			switch (line_number)
			{
				...
				case 2:
					...
					molecule_.number_of_atoms = line_.getField(0);
					...
					{
						molecule_.number_of_substtructures = line_.getField(2).toUnsignedInt();
					}
					if (number_of_fields > 3)
					{
						molecule_.number_of_featuress = line_.getField(3).toUnsignedInt();
					}
					if (number_of_fields > 4)
					{
	...
	{
		Log.info() << "entering readSubstructureSection: line " << number_of_lines_ << endl;
		Size number_of_fields = 1;
		while (nextLine_() && (number_of_fields > 0) && !line_.hasPrefix(TRIPOS))
		{
	...
	void MOL2File::buildAll_(System& system) 
	{
		...
		{
			Log.error() << "MOL2File::read: number of atoms in the MOLECULE section is not consistent the contents of the ATOM section!" << endl;
			return;
		}
		if (bonds_.size() != molecule_.number_of_bonds)
		{
			Log.error() << "MOL2File::read: number of bonds in the MOLECULE section is not consistent the contents of the BOND section!" << endl;
			return;
		}
		if (substructures_.size() != molecule_.number_of_substructures)
		{
			Log.error() << "MOL2File::read: number of substructures in the MOLECULE section is not consistent the contents of the SUBSTRUCTURE section!" << endl;
			return;
		}
		...
		{
			if ((bonds_[i].atom1 > atoms_.size()) || (bonds_[i].atom2 > atoms_.size()))
			{
