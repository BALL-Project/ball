...
// vi: set ts=2:
//
// $Id: colorMeshDialog.C,v 1.19 2003/08/26 09:18:35 oliver Exp $
//
...
namespace BALL
{
	 namespace VIEW
	 {
			...
			{
				System& S = *(System*) &(*rep_->getComposites().begin())->getRoot();
			...
			{
				System& system = *(System*) &(*rep_->getComposites().begin())->getRoot();
			...
			void ColorMeshDialog::loadSettings_()
			{
				...
				for (Index p = 0; p < grid_combobox->count(); p++)
				{
