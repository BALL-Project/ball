...
// vi: set ts=2:
//
// $Id: editOperationDialog.C,v 1.3 2005/06/24 19:06:56 anne Exp $
...
namespace BALL
{
	namespace VIEW
	{
		...
		{
			Log.info() << " Aufruf invalidateComposite" << std::endl;
			//TODO:  loesche aus Undo liste  list_of_operations_
		...
		{	
			Log.info() << " Aufruf removeEditOperationFromList_(Atom* atom) " << std::endl;
			...
			if (to_delete.size() > 0)
			{
				...
				// now we have to delete all the invalid bonds
					Log.info() << atom->countBonds() << std::endl;
		...
		{	
			Log.info() << " Aufruf removeEditOperationFromList_(Bond* bond)" << std::endl;
