...
namespace BALL
{
				namespace VIEW
				{
								...
								////////////////////////// Alignment  MODEL   ////////////////////////////
								AlignmentControlModel::AlignmentControlModel()
												: QAbstractTableModel(),
												alignment_()
								{	 }

								int AlignmentControlModel::rowCount(const QModelIndex& /*parent*/) const
								{
												return alignment_.rows();
								}

								int AlignmentControlModel::columnCount(const QModelIndex& /*parent*/) const
								{
												// selector / name / sequence
												return 3;
								}

								bool AlignmentControlModel::hasSequenceFor(AtomContainer const* ac)
								{
												for (Position i=0; i<alignment_.rows(); ++i)
												{
																if (alignment_.getSequence(i).getOrigin() == ac)
																				return true;
												}
								...
								boost::shared_ptr<Sequence> AlignmentControlModel::getSequenceFor(AtomContainer const* ac)
								{
												boost::shared_ptr<Sequence> result;
												for (Position i=0; i<alignment_.rows(); ++i)
												{
																if (alignment_.getSequence(i).getOrigin() == ac)
																				//TODO implement
																{//result = boost::shared_ptr<Sequence>(alignment_->getSequence(i));
																}
												}
								...
								void AlignmentControlModel::removeSequence(boost::shared_ptr<Sequence> const& sequence)
								{
												//search for the Position of the sequence
												for(int i=0;i< alignment_.rows();i++)
												{
																if((alignment_.getSequence(i)) == *sequence)
																{
																				alignment_.deleteSequence(i);
																				return;
																}
												}
								}

								QVariant AlignmentControlModel::data(const QModelIndex& index, int role) const
								{
												if(index.column() == 0) {
																switch(role) {
																				case Qt::CheckStateRole: {
																...
																switch(role) {
																				case Qt::DisplayRole:
																								return QVariant::fromValue(QString(alignment_.getSequence(index.row()).getName().c_str()));
																}
												} else {
																switch(role) {
																				case Qt::DisplayRole:
																								return QVariant::fromValue(QString(alignment_.getStringSequence(index.row()).c_str()));
																				case Qt::FontRole:
																								{
																												QFont font("Monospace");
																												font.setStyleHint(QFont::TypeWriter);
								...
								QVariant AlignmentControlModel::headerData(int section, Qt::Orientation orientation, int role) const
								{
												switch (role)
												{
																case Qt::DisplayRole:
								...
								Qt::ItemFlags AlignmentControlModel::flags(const QModelIndex& index) const
								{
												if (index.column() == 0)
																return Qt::ItemIsSelectable | Qt::ItemIsUserCheckable | Qt::ItemIsEnabled;
												else
								...
								bool AlignmentControlModel::setData(const QModelIndex& index, const QVariant& data, int role)
								{
												if (index.column() == 0 && role == Qt::CheckStateRole && index.row() < (int)selection_.size())
												{
																selection_[index.row()] = !selection_[index.row()];
								...
								void SequenceControl::buildContextMenu_()
								{
												...
												align_selected_action_ = context_menu_.addAction(tr("Align Selected"), this, SLOT(alignSelected()));
								...
								void SequenceControl::showGuestContextMenu(const QPoint& pos)
								{
												...
												if(sequences_per_tab_["all_sequences"]->sequences_.empty())
												{
																return;
												}

												// if there is already an alignment
												if(tabs_per_name_.getSize() > 1)
												{
													return;
												}
								...
								//TODO 
								void SequenceControl::alignSelected()
								{
												//retrieve selected Sequences
												SequenceControlModel::SequenceVector::iterator seq_it = sequences_per_tab_["all_sequences"]->sequences_.begin();
												std::vector<bool>::iterator sel_it = sequences_per_tab_["all_sequences"]->selection_.begin();


												//Store them in a SequenceVector
												SequenceControlModel::SequenceVector seq_vector;
												for(;seq_it != sequences_per_tab_["all_sequences"]->sequences_.end(); seq_it++, sel_it++)
												{
																if(*sel_it)
																{
																				seq_vector.push_back(*seq_it);
																}
												}


												//if no Protein was selected do nothing or if only one rotein was selected do nothing
												if (seq_vector.empty() || seq_vector.size() == 1) 
												{
																setStatusbarText((String)tr("At least two Sequences must be selected to perform an Alignment!"), true);
																return;
												}

												// add a new AlignmentTab	
												addAlignmentTab("Alignment");

												//add all selected Sequences to newly created AlignmentTab's alignment
												for(SequenceControlModel::SequenceVector::iterator sel_seq_it= seq_vector.begin(); sel_seq_it != seq_vector.end(); sel_seq_it++)
												{
																alignment_per_tab_["Alignment"]->addSequence(*sel_seq_it);
												}

												//align selected sequences
												AlignAlgorithm*	algo = new AlignAlgorithm();

												if(!algo->align((alignment_per_tab_["Alignment"]->alignment_)))
												{
																setStatusbarText((String)tr("Error: Alignment could not be computed!"), true);
																return;
												}


												//tabs_per_name_["Alignment"]->resizeColumnToContents(seq_vector.size());
								...
								void SequenceControl::addAlignmentTab(String const& name)
								{
												//create a new tab
												//	sequences_per_tab_[name] = boost::shared_ptr<SequenceControlModel>(new SequenceControlModel());

												alignment_per_tab_[name] = boost::shared_ptr<AlignmentControlModel>(new AlignmentControlModel());

												QTableView* alignment_view = new QTableView();

												alignment_view->setModel(alignment_per_tab_[name].get());
												alignment_view->horizontalHeader()->setStretchLastSection(true);
												alignment_view->horizontalHeader()->setResizeMode(2, QHeaderView::ResizeToContents);
												alignment_view->horizontalHeader()->setDefaultAlignment(Qt::AlignLeft);

												alignment_view->resizeColumnToContents(0);

												tabs_per_name_[name] = alignment_view;
												tab_widget_->addTab(alignment_view, name.c_str());
