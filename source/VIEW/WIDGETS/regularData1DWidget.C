...
// vi: set ts=2:
//
// $Id: regularData1DWidget.C,v 1.3 2003/09/19 18:18:01 amoll Exp $
//
		...
		data_(0)
{
	...
	Log.error() << "new RegularData1DWidgetMessage " << this << std::endl;
#endif
}
...
{
#ifdef BALL_VIEW_DEBUG
	Log.error() << "new RegularData1DWidget " << this << std::endl;
#endif
...
{
#ifdef BALL_VIEW_DEBUG
	Log.error() << "destroying RegularData1DWidget " << this << std::endl;
#endif
}
...
{
#ifdef BALL_VIEW_DEBUG
	Log.error() << "RegularData1DWidget " << this << " onNotify " << message << std::endl;
#endif
	if ( data_ == 0 ||
