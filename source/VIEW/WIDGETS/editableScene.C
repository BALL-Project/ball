		...
		{	
			...
			if (operationType == ADDED__ATOM)
			{
				...
				if (cm.has(atom))	
				{
					...
					es->notify_(new CompositeMessage(parent->getRoot(), CompositeMessage::CHANGED_COMPOSITE_HIERARCHY)); 
				}
			} 
			// delete a bond
			else if (operationType == ADDED__BOND)
			{
				...
				es->notify_(new CompositeMessage(*atom, CompositeMessage::CHANGED_COMPOSITE_HIERARCHY));
			}
			//change type
			else if (operationType == MERGED__MOLECULES)
			{
				...
				else
				{
					...
					//update representation
					es->notify_(new CompositeMessage(new_molecule->getRoot(), 
					...
					// for all atoms
					for (Position i = 0; (i < molecule.size()); i++)
					{
						// for all bonds
					...
					{
						// delete all bonds
						for (Position i = 0; i < connecting_bonds.size(); i++) 
						{
							...
							// update representation
							scene->notify_(new CompositeMessage(*atom, 
										CompositeMessage::CHANGED_COMPOSITE_HIERARCHY));
						}
					}
				}
...
printf("asdasdasdasdas");
		...
		void EditableScene::initializeWidget(MainControl& main_control)
			throw()
			{
				...
				edit_id_ = insertMenuEntry(MainControl::DISPLAY, "&Edit Mode", this, 
						SLOT(editMode_()), CTRL+Key_E, -1);
				setMenuHint("Switch to edit mode");

				Scene::initializeWidget(main_control);

				scaling_id_ = main_control.insertMenuEntry(MainControl::DISPLAY, 
						 						"&Show Scaling", this, SLOT(showScaling_()), CTRL+Key_A, -1);
				setMenuHint("Display scaling axes");
		...
		void EditableScene::mousePressEvent(QMouseEvent* e)
		{
			...
			if (current_mode_ == (Scene::ModeType)EDIT__MODE)
			{
				...
				{	
					//we want to insert a new atom
					leftButtonPressEvent_(e);
					return;
				}
				if (e->button() == Qt::RightButton)
				{
					//search for an object, that is close to the click ray and open a special context menu
					rightButtonPressEvent_(e);
					return;
				}
				if (e->button() == Qt::MidButton)
				{
					// if there is an atom in select_atom_limit, a new bond 
					// from this atom is started inclusive the bond sketch
					midButtonPressEvent_(e);
					return;
				}
			}
		...
		void EditableScene::mouseMoveEvent(QMouseEvent *e)
		{
			...
			{
				// we have to sketch the bond, snap into the nearest atom
				// and show the actual bond length
				moveEventBondMode_(e);
			}
			...
			// maybe Scene wants to do anything with this event as well. 
			// and if we're not in BOND__MODE, we let it... :-)
			if (current_mode_ != (Scene::ModeType)BOND__MODE)
		...
		//  In the mouseReleaseEvent there are several things to check before
		//  beeing able to insert a bond:
		//  We just have to do something if the system is in bondmode
		//  If so, we have to delete the last bond sketch and the bondlength.
		//  If a second bond partner was found, we to insert a bond and 
		//  eventually merge two atomContainers
		//  If not, 
		//
		void EditableScene::mouseReleaseEvent(QMouseEvent *e)
		{
			...
			if (current_mode_ == (Scene::ModeType)BOND__MODE)
			{
				...
				if (atom)
				{
					releaseEventBondModeWithAtom_(atom);
				}
				else // no atom found
				{
					releaseEventBondModeWithoutAtom_(e);
				}
		...
		void  EditableScene::selectAtom()
		{
			...
			{
				atom->select(); 					
				notify_(new CompositeMessage(*atom, CompositeMessage::SELECTED_COMPOSITE));
			}
		}
		...
		void  EditableScene::deselectAtom()
		{
			...
			{
				atom->deselect(); 					
				notify_(new CompositeMessage(*atom, CompositeMessage::DESELECTED_COMPOSITE)); 
			}
		}
      ...
      *context_menu_composite_, CompositeMessage::CHANGED_COMPOSITE_HIERARCHY);
		...
		// inserts a new atom 
		void EditableScene::leftButtonPressEvent_(QMouseEvent* e)
		{
			...
			std::vector<Atom*> v; 
			EditOperation eo(a, NULL, v, "Added atom of type " 
					+ PTE[edit_atom_type_].getName() + " at position (" 
		...
		void EditableScene::rightButtonPressEvent_(QMouseEvent* e)
		{
		...
		// if there is an atom in select_atom_limit, a new bond from this atom is started
		// the new bond is sketched with a thin line => we have to update the corresponding 
		// bond_positions (	x_ewindow_bond_pos_first_ = pos.x and x_ewindow_bond_pos_second_old_ = pos.x)
		void EditableScene::midButtonPressEvent_(QMouseEvent* e)
		{
		...
		void EditableScene::moveEventBondMode_(QMouseEvent* e)
		{
	 	...
	 	/**
	 	*  this function performs the action of mouseMoveEvent in Bond Mode
		*  this code is just copied form the regular move mode
	 	*/
	 	void EditableScene::moveEventMoveMode_()
		{
			if(RTTI::isKindOf<Atom>(*context_menu_composite_))
			{
				Atom* atom  = dynamic_cast<Atom*>(context_menu_composite_);
		 ...
		 *  having found a second bond partner. First, we have to check if first and second 
		 *  bond partner are the same atom. If not, we insert a bond between them. 
		 *  If the two atoms belong to different atomContainers, we have to merge the
		 *  atomContainers and emit the corresponding undo-operation
		 */
...
//   			highlightAtomContainer_(current_atomContainer_);
		 ...
		 */
		void EditableScene::releaseEventBondModeWithoutAtom_(QMouseEvent* e)
		{
			...
			if (first_atom_for_bond_ != 0)
			{
				...
				{		
					...
					EditOperation eo(a, NULL, v, "Added atom of type " + PTE[edit_atom_type_].getName() 
							+ " at position (" 
							+ String(atom_position.x) + ", "
							+ String(atom_position.y) + ", "
							+ String(atom_position.z) + ")", EditOperation::ADDED__ATOM);
					...
					// set the bond and update representation
					...
					notify_(new CompositeMessage(*a, CompositeMessage::CHANGED_COMPOSITE_HIERARCHY));	
				}
				else
				{
		...
		void EditableScene::highlightAtomContainer_(AtomContainer* ac)
		{
			List <Composite*> cl;
			cl.push_back(ac);
		...
		// this function projects the 3D view plane to 2D screen coordinates
		...
		Vector3 EditableScene::clickedPointOnViewPlane_(int x, int y)
		{
			...
			s_ = Vector3(p_ * ( (getStage()->getCamera().getLookAtPosition() 
							- getStage()->getCamera().getViewPoint()).getLength()
		...
		//
		// slot for communication with undoDialog
		//
		...
		{	
			...
			// once painted ruler axis (painted with the first atom) remains till the
			// editmodus is quit
			...
			//we just paint the scaling axes if we are in rotateMode or in moveMode 
			if ( (!show_scaling_axes_) || (current_mode_ == (Scene::ModeType)PICKING__MODE))
			{
				return;
			}
			...
			if ( (vertical_distance < 5) || (horizontal_distance < 5) )
			{
				return;
			}	
		...
		/**
		 * This function defines the rules for searching the atomContainer, in which a newly
		 * created atom will be inserted.
		 * Since we assume by construction the structure 
		 *  		System->molecule->atom  
		 * the function just returns the parent, which is a molecule. 
		 * This method must be more sophisticated if we allow creation of a 
		 * more complex structure.
		 */
		AtomContainer* EditableScene::getAtomContainer_(const Atom* atom)
		{
			return RTTI::castTo<AtomContainer>(*(atom->getParent()));
		}
