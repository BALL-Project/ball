// $Id: forwardIterator.h,v 1.1 1999/08/26 07:53:16 oliver Exp $
...
#	include <BALL/CONCEPT/forwardManipulator.h>
#endif
...
namespace BALL {
	...
	*/
	...
	class ForwardManipulator
		: public ForwardManipulator_<Container, DataType, Position, Traits>
	{
		public:
		...
		*/
		ForwardManipulator()
			:	ForwardManipulator_<Container, DataType, Position, Traits>()
		{
		}
	
		/**
		*/
		ForwardManipulator(const ForwardManipulator &manipulator)
			:	ForwardManipulator_<Container, DataType, Position, Traits>(manipulator)
		{
		}

		/**
		*/
		ForwardManipulator(const BaseIterator<Container, DataType, Position, Traits> &iterator)
			:	ForwardManipulator_<Container, DataType, Position, Traits>(iterator)
		{
		}
		//@}
		...
		ForwardManipulator operator ++ (int)
		{
			ForwardManipulator manipulator(*this);
			++(*this);
			return manipulator;
		}
		...
		{
			return ((ForwardManipulator*)this)->findFirst(predicate);
		}
		...
		{
			return ((ForwardManipulator*)this)->findNext(predicate);
		}

		static ForwardManipulator begin(const Container &container)
		{
			ForwardManipulator manipulator(container);
			manipulator.toBegin();
			return manipulator;
		}

		static ForwardManipulator end(const Container& container)
		{
			ForwardManipulator manipulator(container);
			manipulator.toEnd();
			return manipulator;
		...
		ForwardManipulator(const Container& container)
			:	ForwardManipulator_<Container, DataType, Position, Traits>(container)
		{
		}
