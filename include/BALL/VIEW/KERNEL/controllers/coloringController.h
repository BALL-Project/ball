	...
	namespace VIEW
	{
		...
		class BALL_VIEW_EXPORT ColoringController : public Controller
		{
			...
			public:
				...
				/**
				 * Reset the coloring settings to the method-defined defaults
				 * (element coloring, 0..100 value range) in a SINGLE §2 event
				 * (999.64 reset path — NOT the legacy two-step revert(); apply()).
				 * Sets the mirror to the defaults, emits the change notifications,
				 * then runs one apply() that captures current owner state as
				 * `before`, installs the defaults as `after`, emits exactly one
				 * event and records one reversible payload. Overrides
				 * Controller::reset().
				 */
				void reset() override;
