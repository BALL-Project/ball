	...
	namespace VIEW
	{
		...
		class BALL_VIEW_EXPORT StereoController : public Controller
		{
			...
			public:
				...
				/**
				 * Reset the stereo settings to the method-defined defaults
				 * (disabled, zero eye/focal distance, no swap) in a SINGLE §2
				 * event (999.64 reset path — NOT the legacy two-step revert();
				 * apply()). Sets the mirror to the defaults, emits the change
				 * notifications, then runs one apply() (one event, one reversible
				 * payload). Overrides Controller::reset().
				 */
				void reset() override;
