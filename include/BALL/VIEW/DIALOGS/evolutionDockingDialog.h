	...
	namespace VIEW
	{
        ...
        //@}
				...
				/** Fetchs the preferences from the INIFile.
					...
					* Calls \link EvolutionDockingDialog::fetchPreferences_ fetchPreferences_ \endlink to read the redocking options.
			...
			public slots:
				...
				void show();
			 	...
			 	/** Is called when reset button is pressed.
				 	* Calls \link EvolutionDockingDialog::reset reset \endlink.
				 	*/
				void reset();
		   ...
		   void showForceFieldOptions();
			...
			protected:
				...
				/** Function to read the redocking options from INIFile into vector backup_.
					* If INIFile has not yet section <b> GEOMETRIC_FIT_OPTIONS_REDOCK </b>, 
					* fill vector \link DockDialog::backup_ backup_ \endlink with default values.
				 ...
				 *  Is called in \link EvolutionDockingDialog::show show \endlink if \link EvolutionDockingDialog::has_changed_ has_changed_ \endlink 
				 *  is true and in \link EvolutionDockingDialog::writePreferences writePreferences \endlink if 
				 *  \link EvolutionDockingDialog::is_redock_ is redock_ \endlink is true.
				 */
			...
			private:
				...
				/** Needed to guarantee that both, docking and redocking preferences can be written to INIFile
					...
					* In \link EvolutionDockingDialog::fetchPreferences fetchPreferences \endlink, we read the last redocking options from INIFile in this vector 
					* and in \link EvolutionDockingDialog::writePreferences writePreferences \endlink, we write the redocking options in INIFile from this vector
					*/
