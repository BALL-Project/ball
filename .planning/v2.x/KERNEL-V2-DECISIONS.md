...
| D7 | Reference-return getters | **Preserved with a documented (opt-in-enforced) live-reference contract.** `Vector3& getPosition()` returns `&store->positions[idx_]`. Reference is valid only while no store-resizing operation (`allocate_atom` beyond capacity, `reserve` growing capacity, `compact`, `insert`/`remove`) is called on the same store. Generation check on the *handle* (D8) catches stale handles; it cannot catch already-escaped `Vector3&`. The `BorrowedColumnRef` RAII helper + per-store `borrowed_ref_count_` are an **opt-in** debug guard for callers that want machine enforcement; the raw `Atom::getPosition/Velocity/Force` getters do NOT bump the counter (auto-wrapping their return value would change the v1.x-compatible signature surface). Without opt-in, dangling references are undefined behaviour in both debug and release. [Audited K0.4.7 (2026-05-18) per Codex Round 4 HIGH-5; K0.5/K0.6 will migrate hot internal callers that want the guard.] [Originally amended 2026-05-17 per Codex Round 2 HIGH-D7.] |
...
| D11 | Compiled selection | **DONE — K0.5.0-K0.5.6 complete** (commits 5356fe188..8f318d96a, 2026-05-18). Variant-based cached AST + per-store LRU cache + Expression-aware compile (OwnedPred wraps ring/SMARTS/user-registered) + parser mutex + benchmark. **Median 50.7× speedup** across 4 representative fast-path queries on 50k atoms (range 19.7×–86.0×); >=10× design gate cleared with 5× margin. OwnedPred slow path is ~1.4× faster than v1.x (one back_ptr deref + same vcall). K0.7 perf phase re-verifies at 100k. |
| D12 | Persistence format | **DONE — K0.6.0-K0.6.5c complete** (commits ae22fe260..b8 [pending K0.6.5c commit], 2026-05-18). PIVOTED 2026-05-18 from custom-binary (original D12 spec) to JSON via vendored nlohmann::json — typical workloads ≤10k atoms don't justify 3-4-week binary budget; JSON wins on debuggability + schema evolution + zero endianness work; binary fast-path can be added in K0.7 if profiling demands. Coverage: per-column SoA atom store (positions/velocities/forces/charges/radii/atom_types/formal_charges/element_indices/selection/names/type_names/stable_ids/is_freed/properties), bond table (live records only; tombstones stripped), per-atom + per-molecule + per-system PropertyManager bags (BOOL/INT/UNSIGNED_INT/FLOAT/DOUBLE/STRING fully round-trip, OBJECT/SMART_OBJECT placeholder), orphan-atom (System direct child) path, MAJOR/MINOR version policy, document_type discriminator, bit-exact float opt for trajectory replay, comprehensive parse-error boundary, stable_id round-trip via friend-restricted bulk-restore. Two Codex rounds (R7 + R8) cleared. K0.6.4 (v1→v2 converter from .bal/PDB/HIN) deferred to Track B (gated on FORMAT re-enable). 42 K0.6 sub-tests + 13 kernel + 4 v1.x expression tests green. |
| D13 | Memory budget gate | **PARTIAL — measured K0.7.3 (commit 7fa7c32d2):** SoA-column slice = 139 B/atom (under 160 B budget ✓); full-live-handle workload = 499 B/atom (3.1× over budget). The over-budget portion is `sizeof(Atom)`=360 B + `sizeof(Bond)`=288 B, dominated by D2/D3/D4 thin-stub base classes (Composite + PropertyManager + Selectable). Full reduction (≈64 B handle) requires actual D2/D3/D4 removal, deferred to v2.1. v2.0 ships with the gap documented in RELEASE-NOTES-v2.0.md; workloads where atoms live in the store without per-atom heap handles (trajectory frames, batch analysis, persistence-only paths) meet D13. Workloads that materialise an `Atom*` per atom (interactive editing, GUI selection, v1.x-compat code paths) do not. [Original spec amended 2026-05-17 per Codex Round 2 BLOCKER-2; PARTIAL classification + measurement added 2026-05-18 per Codex Round 9.] |
...
| D17 | Orphan-store mutex | **`std::recursive_mutex` (V21-ORPHAN-MUTATOR-LOCK, 4b01405b5).** Atom() ctor + dtor + every `writeStoreXxx_` setter holds it when bound to orphan. Recursive because the ctor's initial-writes block (R11 fix A) wraps calls into the same helpers. Per-System stores are single-thread-per-store by convention (no per-store mutex). Pre-reserves 64k slots on first orphan-store use (R11) so the typical workload never reallocates and the mutex covers only the slot-allocation contention. [Added 2026-05-18 per Codex R11 fix A + V21-ORPHAN-MUTATOR-LOCK + R13 sanity check.] |
| D18 | Per-store bond mutex | **`std::mutex` `bond_mutex_` per MoleculeStore (V21-BOND-MUTEX, 0b77d65e5).** Protects `add_bond`, `remove_bond`, `remove_bonds_between`, `swap_atom_connectivity`, and `ensure_csr_()`. Reads via `bond(i)`/`bonds_of(i)`/`for_each_bond_of(i, fn)` lock-free; caller must keep returned references valid by not invoking concurrent bond mutators (D7 pattern). Orphan-store bonds aren't a contention point (default Atom() doesn't bond). [Added 2026-05-18 per Codex R12 K9 + R13 sanity check.] |
| D19 | CompiledExpression staleness | **Per-evaluate generation check (V21-COMPILED-EXPR-GENERATION-CHECK, 38453fecf).** CompiledExpression captures `(store_*, generation_at_compile)` in its ctor; `evaluate()` throws `Exception::InvalidArgument` on store mismatch or generation bump. The cache (`CompiledExpressionCache`) invalidates entries on `~MoleculeStore` and on `compact()`, but the new check protects `shared_ptr<CompiledExpression>` instances held outside the cache (e.g., in mid-evaluation by another thread) from operating on stale `intern_name` offsets after `compact()` rebuilt the string pool. Caller catches and recompiles. [Added 2026-05-18 per Codex R13.2.] |
| D20 | String pool reclamation | **`compact()` rebuilds `string_pool_` + `string_intern_` from live `name_strings_` / `type_name_strings_` columns (V21-STRING-POOL-COMPACT, 25bfcd58e).** Pre-fix, the pool was append-only between explicit `clear()` calls; long-running processes leaked monotonically-growing unique name space. Now `compact()` reclaims dead-slot strings. Also bumps `generation_` unconditionally (R12 K5) and invalidates the cache (R12/R13). [Added 2026-05-18 per Codex R12 K4 + R13.2.] |
| D21 | Stable-ID overflow policy | **`next_stable_id_` is uint64; overflow throws `Exception::OutOfMemory` rather than wrapping (V21-STABLE-ID-OVERFLOW, 853e94da4 + R13.4 38453fecf hoist).** `next_stable_id_alloc_()` is called BEFORE any state mutation in `allocate_atom_with_back_ptr_` so an overflow throw leaves the store at its pre-call shape. `restore_stable_ids_for_load_()` accepts `max_id == UINT64_MAX` without wrapping; next allocate throws cleanly. At 1 ns per allocation, 64-bit space takes ~584 years to exhaust — exhaustion is reachable only via adversarial JSON or careless tests. [Added 2026-05-18 per Codex R12 K7 + R13.4.] |
