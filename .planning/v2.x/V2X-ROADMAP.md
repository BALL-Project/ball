...
| **H2** mutation-mirror + traversal | ✅ DONE (H2d close, Codex H2d-R1 GO) | **H2a ✅ R36d GO**: forward-only v0→table mirror for insert (adoption materialisation) + remove, with the `being_destroyed_` destruction guard (defuses the P2.1.1 heap-corruption trap). **H2b ✅ (topology)** `103994a26`: splice/swap/clear/replace **topology** mirror DONE + **fixed a latent legacy `Composite::swap` sibling infinite loop** (snapshot relink); 286/286. **SCALAR-field mirroring (name/id/insertion/SS-type) DEFERRED to HCP-1** (built once against the collapsed role+payload model — see `H1b′`); remaining for H2b/H2c: `insertParent`, container `setProperty`, atom `select`. **H2d ✅**: randomized full-surface parity sweep (300 deterministic steps over append/move/positional/swap/clear/remove/replace-rooted→rooted/select/deselect/post-root scalar setters; full scalar-bearing descriptor parity + root selection-count parity after every step) in `HierarchyParity_test`; Codex **H2d-R1 GO**; 286/286 + 100/100 stress. The sweep also confirmed the **HCP-2c carry-over boundary** (replace-with-an-orphan member → materialise-new-member mirror) is genuinely still pending. **H2c folds into HCP-2** (handle-yielding `AtomIterator`/container iterators/`apply` are built once as role-filtered iteration, not typed-then-collapsed). **DEFERRED to HCP-2c (documented, NOT closed):** replace/insertParent materialise-new-member, rooted full-subtree-replacement (`set`/`operator=`/`persistentRead`), container `setProperty` role-refinement. |
...
**HCP-1 status (DONE):** role columns (HCP-1a), scalar mirror (HCP-1b.1/.2),
...
- **Full-subtree-replacement on an already-ROOTED object** — `set()`/
  `operator=`/`persistentRead()` are deep (deep=true) ops that replace
  children+scalars; the mirror doesn't re-materialise on a rooted target
  (KR1 HIGH-1 cross-store move + insertParent/`replace` materialise-new-member
  belong here too). Common build-then-root usage is correct (materialise
  captures). **H2d must not randomize these onto rooted objects** until the
  full-replacement mirror lands.
- **Container `setProperty`/`clearProperty` mirror** (incl. property-driven
  `ResidueKind`/`IS_SOLVENT` → role refinement) — R32 carry-over; lands with
  the role-property mirror at HCP-2.
KR1/HCP1R confirmed sound otherwise (swap relink, vector-only `clear_children`,
`being_destroyed_` guard, handle/payload compactness, shared scalar dispatch,
materialise idempotence).
