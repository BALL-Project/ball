...
**State carried in from v1.6.2 (2026-05-17 user feedback after initial v1.6.2 release shipped `.zip`):**
  - **macOS DMG packaging is now WIRED in `release.yml`** via a plain `hdiutil create -format UDZO` stopgap (the `Create DMG (macOS)` step replaces the prior `Zip .app (macOS)` step). v1.6.2 iteration-5 ships as `BALLView-v1.6.2-macos-arm64.dmg`. User direction: **never ship .zip for macOS again** — DMG is the canonical Mac distribution format and is non-negotiable going forward.
  - **Phase 8's `create-dmg` upgrade still applies on top of this stopgap.** `create-dmg` wraps `hdiutil` to add: (a) Applications-folder symlink for the drag target, (b) BALL-branded background image with drop-zone visual cues, (c) iconified DMG title bar. The plain `hdiutil` stopgap just produces a generic DMG that mounts to `/Volumes/BALLView` showing `BALLView.app` — functional but unbranded. Phase 8 upgrades to `create-dmg` + the signed/notarized cert chain in the same workflow surgery.
  - Cross-ref the v1.6.2 release.yml shape so Phase 8's create-dmg integration knows the per-step contract (hdiutil sits AFTER macdeployqt + data-bundle + ad-hoc-resign; create-dmg slots into the same position).
**References**:
