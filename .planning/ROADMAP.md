...
### Phase 999.65: Headless representation-builder extraction (TARGETED FOR v1.7.x · 3rd PREREQUISITE FOR 999.53)

**Goal:** Make `displayProperties`/`modelSettingsDialog`/`coloringSettingsDialog` deletable by relocating the representation-creation **orchestration** off `DisplayProperties` onto a dialog-free `RepresentationBuilder`. Surfaced by the 999.53 audit: `DisplayProperties` is still the live engine for the project's core load→render path — `onNotify(NEW_MOLECULE)` auto-creates the default representation, `RepresentationManager::restoreRepresentations` calls `dp->createRepresentation` (representationManager.C:438), and `MainControl::enableCreationForNewMolecules` (mainControl.C:2115-2177) toggles it. 999.57 relocated only the processor *factories* (`ModelProcessorFactory`/`ColorProcessorFactory`); the orchestration that calls them to build a full `Representation` still lives only on the dialog.

**Scope:** extract `createRepresentation` + `applyModelSettings_`/`applyColorSettings_` orchestration into a headless `RepresentationBuilder` (consuming the 999.57 factories); repoint `onNotify(NEW_MOLECULE)`, `RepresentationManager::restoreRepresentations`, and `enableCreationForNewMolecules` onto it. Behavior-preserving (molecule load + project restore render identically).
**Canonical source:** the 999.57-03-SUMMARY sequencing-gate note + the 999.53 planner finding.
...
**Goal:** Delete the legacy VIEW dialog files whose Inspector replacements have shipped (deferred from Wave 4 Phase 999.48 §10, breadcrumb commit `7f56f80d89`), confirming each is UNREFERENCED before deletion and compile-checking between deletion groups. **Re-audit (2026-05-22) outcome:** 6 of the 9 are deletable now; the displayProperties/modelSettingsDialog/coloringSettingsDialog triple stays blocked (still the live representation-creation engine) — VIEW-CLEAN-01 lands PARTIAL.
...
**Depends on:** Phase 999.51, Phase 999.52, **Phase 999.57, Phase 999.58** (all landed). **Effort:** small–medium (3 live-ref migrations + 6-triple deletion + CI validation).
**Plans:** 2 plans, 2 waves.
- [ ] 999.53-01-PLAN.md — Clear the 3 remaining live refs: geometricControl stack-ClippingDialog → direct clip; mainframe LabelDialog registration removed (Inspector LabelSection supersedes); modifyRepresentationDialog migrated off the embedded MaterialSettings widget. (VIEW-CLEAN-01, wave 1)
- [ ] 999.53-02-PLAN.md — Delete the 6 unreferenced dialog triples (light/stage/stereo/clipping/label/material) in 3 compile-checked groups + drop their sources.cmake entries + delete lightSettings.sip & its VIEW.sip %Include + scrub stale comment breadcrumbs; blocking human-verify smoke (render + Inspector sections + legacy dialogs gone). (VIEW-CLEAN-01, wave 2)

**⚠ NOT in this phase (genuinely blocked — needs a follow-on):** the `displayProperties` / `modelSettingsDialog` / `coloringSettingsDialog` triple + `LegacySettingsHelper` + the `Tools › Legacy Settings` submenu. `DisplayProperties` is still the live representation-creation engine (NEW_MOLECULE auto-rep + project-restore `createRepresentation` + `enableCreationForNewMolecules`); no headless representation-builder exists to take it over. Per 999.57-03-SUMMARY's sequencing gate, a follow-on phase must extract that builder first. So **VIEW-CLEAN-01 closes PARTIAL** here.
