...
CONCEPT modules. Replace with type-safe alternatives.

**Sub-phases:**

| # | Subject | Sites | Strategy |
|---|---|---|---|
| P3.1 | Audit current sites + classify | 24 | Read each site; classify as (a) hot path needing fast-check, (b) cold path, (c) genuinely needs runtime type-discrimination |
| P3.2 | Remove hot-path RTTI via overload | TBD count | Where the calling context can know "this is an Atom" statically, refactor to take `Atom*` directly |
| P3.3 | Remove cold-path RTTI via virtual | TBD count | Add a `bool Composite::isAtom_() const { return false; }` virtual; Atom overrides to true. Replace `dynamic_cast<Atom*>(c)` with `c->isAtom_() ? static_cast<Atom*>(c) : nullptr`. |
| P3.4 | Remove genuine-discrimination RTTI | TBD count | The "visitor wants to handle Atom specially among Composite children" case. Add a virtual dispatch hook like `Composite::accept(AtomVisitor&)`. |
| P3.5 | Update SIP Python bindings | n/a in CORE_ONLY | Defer — PYTHON not in CORE_ONLY surface; v2.1 backlog if needed |
| P3.6 | Close review (R23) | — | Codex reviews removal completeness + correctness |
...
must pass. Add new tests where RTTI-handling exposed
behavioral edges.
...
| # | Subject | Deliverable |
|---|---|---|
| P4.1 | V21-LOAD-BATCH | JSON load batches atom row construction; defers Composite-tree insertion until all atoms allocated. Closes 15× save/load gap to ≤2×. New benchmark in JsonBench_test. |
| P4.2 | V21-BOND-PROPERTY-JSON | Serialise Bond handle's PropertyManager bag (MMFF94SBMB, MMFF94RBL, VIRTUAL__BOND, HBondProcessor annotations) in K0.6 JSON. New `bond_property_columns` schema section per D30a MINOR bump. |
...
| P4.5 | Close review (R25) | Codex reviews JSON round-trip correctness + backward read |

**Risk:** V21-LOAD-BATCH changes the JSON loader fast path —
introduces a new performance-sensitive code path. Mitigation:
JsonBench_test gates the perf claim; existing
MoleculeStoreJson_test verifies correctness.
...
| P5.2 | V21-CI-PERF-GATES | Calibrate JsonBench / SelectorBench / MemoryBudget gates against CI machine variance to detect ~2× regression instead of 100× (current). Median of N=5 runs. |
| P5.3 | V21-MEDIAN-OF-N-BENCH | Bench harness refactor to report median + p99 across N runs; release-claim numbers reproducible. |
| P5.4 | Generation-guard `BALL_DEBUG` mode | Per D25: in `BALL_DEBUG` builds, every Atom handle deref asserts `handle.generation == store->generation_`. Free in release. |
| P5.5 | Close review (R27) | Codex reviews perf gates + iter API completeness |
...
| # | Subject | Deliverable |
|---|---|---|
| P6.1 | RELEASE-NOTES-v2.1.md | Draft; cover D39 deferral + scaffolding + RTTI + JSON + perf |
...
| R26 | P5 planning |
| R27 | P5 close |
| R28 | v2.1.0-rc1 pre-tag |

R21 needs to GO before P3 starts. Per the convergence pattern
(R20→R20d took 4 rounds), expect 2-4 iterations.
