...
status: executing
stopped_at: Phase 5 Plan 04 complete (CI matrix to Qt 6 + Qt5 holdout lint)
...
**Plans:** 4 of 8 complete (05-01, 05-02, 05-03, 05-04)
**Status:** Executing Phase 05
**Progress:** [█████░░░░░] 50% of Phase 5; overall 96% of v1.6 active phases
...
**Last action:** Phase 5 Plan 04 (CI matrix + Qt5 holdout lint) executed — 2 files: NEW `scripts/check-no-legacy-qt6-symbols.sh` (Phase 2 grep-gate scaffold reused verbatim with a Qt5-symbol LEGACY_PATTERN, scans source/ + include/BALL/, exits 0 against current tree + 1 against injected QRegExp canary); `.github/workflows/ci.yml`: Linux matrix runner ubuntu-22.04 → ubuntu-24.04 (D-02), Qt5 -dev apt packages stripped + new jurplel/install-qt-action@v4 step pinned to `version: '6.5.*'` (modules qtbase/qttools/qtopengl, cache: true) — D-01 mitigation for apt 6.4.2 on 24.04 (Pitfall 1); macOS brew install qt@5 → qt (matches Plan 05-01 D-03); smoke-check stdout teed to /tmp/ballview-smoke.log + new `Assert GL compat profile` steps grep BALLVIEW_GL_DIAG for `gl_profile=compatibility` on macOS + Linux (D-06 + D-08 runtime oracle — blank PNG alone is insufficient under Qt 6); lint job bumped to ubuntu-24.04 + new step runs the Plan 05-04 Task 1 script. Bonus: -DUSE_QTWEBENGINE=OFF added to ci-windows configure (vcpkg manifest does not ship qtwebengine, CMakeLists option defaults ON for backward compat). Static gates green: YAML parses (`yaml.safe_load`), all 18 acceptance-tally greps pass. Runtime D-08 oracle deferred to next plan wave: BLOCKER-A/B/D still gate the BALL link; the CI harness is wired and ready, runtime exercises itself the first push after A/B/D land. Commits: `c04865f` (Task 1: lint script), `a26cafa` (Task 2: ci.yml Qt 6 bring-up + lint hookup + USE_QTWEBENGINE=OFF).

**Stopped at:** Phase 5 Plan 04 complete (CI matrix + Qt5 holdout lint)

**Next action:** **BALL Qt 6 bring-up plan** (implicit between Wave 3 and Wave 4) to land BLOCKER-A (dockResultFile.h → QXmlStreamReader), BLOCKER-B (templated QMutexLocker in mutex.h + TMutex<QMutex> → QRecursiveMutex), and BLOCKER-D (glRenderWindow.h:51-52 `Qt::WindowFlags w_flags = 0` → `Qt::WindowFlags()`). Once that lands, `cmake --build --preset ci-{macos,linux,windows} --target BALL VIEW BALLView` goes link-green, the smoke check fires on macOS + Linux, and the Plan 05-04 `Assert GL compat profile` grep step exercises the D-06/D-08 runtime oracle. After that: Wave 4 — **Phase 5 Plan 05** (GL-core spike: CoreGLRenderer + RendererFactory::Kind::OpenGL_Core + color-buffer FBO picking + spike CI variant).
