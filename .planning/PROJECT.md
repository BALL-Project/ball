...
- **2.0 = substrate modernization.** Carries the post-v1.7 infrastructure transitions: PIPE-01 (renderer pipeline rewrite, fixed-function → QRhi; backlog 999.6) + INIFile-to-YAML config-format migration (backlog 999.9) + TCP-socket → REST API remote-control modernization (backlog 999.10, supersedes 999.3). All three are correctness-sensitive substrate changes that benefit from being grouped under a single major-version bump where users expect breaking-but-documented behavior changes (render backend swap, file-format churn, wire-protocol change). Phase 5 SPIKE-02 locked the GL-Core → QRhi split-pattern for v2; 999.9 and 999.10 keep the v2 theme coherent.
